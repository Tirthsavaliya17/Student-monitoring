...
db.connect((err) => {
  ...
  // Step 1: Add StudentID column if it doesn't exist
  const addStudentIdColumn = `ALTER TABLE Student ADD StudentID INT AUTO_INCREMENT PRIMARY KEY FIRST`;
  
  db.query(addStudentIdColumn, (err, result) => {
    if (err) {
      if (err.code === 'ER_DUP_COLUMNNAME') {
        console.log('✓ StudentID column already exists');
      } else {
    ...
    // Step 2: Create studentmentor table
    ...
    db.query(createStudentMentorTable, (err, result) => {
      ...
      // Step 3: Create studentmentoring table
      ...
      db.query(createStudentMentoringTable, (err, result) => {
        ...
        setTimeout(() => {
          const assignMentor = `
            INSERT IGNORE INTO studentmentor (StudentID, StaffID, FromDate, ToDate)
          ...
          db.query(assignMentor, (err, result) => {
            ...
            const insertSession = `
              INSERT IGNORE INTO studentmentoring (StudentMentorID, DateOfMentoring, ScheduledMeetingDate, MentoringMeetingAgenda, AttendanceStatus, StudentsOpinion)
              VALUES (
