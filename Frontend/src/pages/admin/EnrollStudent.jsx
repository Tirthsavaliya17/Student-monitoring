...
const EnrollStudent = () => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch('http://localhost:3000/api/students', {
      ...
      .then(res => {
        if (!res.ok) return res.json().then(err => { throw new Error(err.message || 'Failed to enroll student'); });
        return res.json();
      })
  ...
  return (
    <>
      <div className={styles.header}>
        <div>
          <h1 className="animate-slide-up stagger-1">Enroll New Student</h1>
          <p className="animate-fade-in stagger-2 text-subtle">Enter student details to add them to the system.</p>
        </div>
